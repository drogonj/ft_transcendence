...
#A VOIR POUR REMETTRE LE FICHIER DE CONFIGURATION DE PGPOOL 
sed -i "s/^sr_check_user.*/sr_check_user = '${PGPOOL_SR_CHECK_USER}'/" /opt/bitnami/pgpool/etc/transcendence_pgpool.conf
sed -i "s/^sr_check_password.*/sr_check_password = '${PGPOOL_SR_CHECK_PASSWORD}'/" /opt/bitnami/pgpool/etc/transcendence_pgpool.conf
sed -i "s/^health_check_user.*/health_check_user = '${PGPOOL_POSTGRES_USERNAME}'/" /opt/bitnami/pgpool/etc/transcendence_pgpool.conf
sed -i "s/^health_check_password.*/health_check_password = '${PGPOOL_POSTGRES_PASSWORD}'/" /opt/bitnami/pgpool/etc/transcendence_pgpool.conf
